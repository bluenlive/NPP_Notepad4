...
void DoInjectRegexPresets()
{
    ...
    const std::vector<std::wstring> regexPresets = {
        ...
        L"(?#공백 빈줄)^[ \\t]+$",
        L"(?#연속 개행)(\\r?\\n){2,}",
        L"(?#URL 주소)\\bhttps?://[^\\s/]+(?:/[^\\s]*)?\\b",
        L"(?#이메일 주소)\\b[^@\\s]+@[^@\\s.]+(?:\\.[^@\\s.]+)+\\b",
        L"(?#한글)[가-힣ㄱ-ㅎㅏ-ㅣ]",
        L"(?#이모지·기호)([\\x{2600}-\\x{27BF}]|.[\\x{DC00}-\\x{DFFF}])",
        L"(?#주석 내 이모지)//.*?([\\x{2600}-\\x{27BF}]|.[\\x{DC00}-\\x{DFFF}])",
        L"(?#아스키 사칭 문자)[^\\x00-\\x7Fㄱ-ㅎㅏ-ㅣ가-힣А-яёЁ一-龥ぁ-んァ-ヶÀ-ÿ\\r\\n]|.[\\x{DC00}-\\x{DFFF}]",
        L"(?#한영 제외)[^\\x00-\\x7Fㄱ-ㅎㅏ-ㅣ가-힣\\r\\n]|.[\\x{DC00}-\\x{DFFF}]",
    };
