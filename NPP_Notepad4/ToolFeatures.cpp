...
void DoRemoveTags()
{
    ...
    // 2. 선택 영역 범위 확인
    Sci_Position selStart = ::SendMessage(hSci, SCI_GETSELECTIONSTART, 0, 0);
    Sci_Position selEnd = ::SendMessage(hSci, SCI_GETSELECTIONEND, 0, 0);
    if (selStart == selEnd) return;

    // 3. 선택된 UTF-8 텍스트 가져오기
    Sci_Position lenUtf8 = selEnd - selStart;
    ...
    auto GetBytePos = [&](UINT idx) -> Sci_Position {
        ...
        if (pszTextW[s] == L'<' || bInScript || bInStyle) {
            ...
            while (s < cchTextW) {
                if (pszTextW[s] == L'<') {
                    ...
                    UINT t = 0;
                    UINT peek = s + 1;
                    bool isClosing = (peek < (UINT)cchTextW && pszTextW[peek] == L'/');
                    if (isClosing)
                        peek++;
            ...
            Sci_Position bStart = GetBytePos(hunkStartIdx);
            Sci_Position bEnd = GetBytePos(s);

            // 앞뒤 문맥을 보고 삽입할 구분자(공백/줄바꿈) 결정
            std::string repText = "";
            WCHAR prevChar = (hunkStartIdx > 0) ? pszTextW[hunkStartIdx - 1] : 0;
            WCHAR nextChar = (s < cchTextW) ? pszTextW[s] : 0;
...
void DoRemoveComments()
{
    ...
    auto IsPythonStandaloneDocstring = [](HWND hSci, Sci_Position blockStart) -> bool {
        Sci_Line line = ::SendMessage(hSci, SCI_LINEFROMPOSITION, blockStart, 0);
        Sci_Position lineStart = ::SendMessage(hSci, SCI_POSITIONFROMLINE, line, 0);
        bool isFirstOnLine = true;
        for (Sci_Position p = lineStart; p < blockStart; ++p) {
            int ch = (unsigned char)::SendMessage(hSci, SCI_GETCHARAT, p, 0);
            if (ch != ' ' && ch != '\t') { isFirstOnLine = false; break; }
        }
        Sci_Position p = (isFirstOnLine ? lineStart : blockStart) - 1;
        while (p >= 0) {
            int ch = (unsigned char)::SendMessage(hSci, SCI_GETCHARAT, p, 0);
            int style = (int)::SendMessage(hSci, SCI_GETSTYLEAT, p, 0);
            if (ch == ' ' || ch == '\t' || ch == '\r' || ch == '\n' || style == 1 || style == 12) {
                p--; continue;
            }
    ...
    auto IsCommentStyle = [&](HWND hSci, Sci_Position pos, int style) -> bool {
    ...
    auto IsBlockCommentStyle = [](int style) -> bool {
        switch (g_cachedLangType) {
        case L_C:
        case L_CPP:
            switch (style) {
    ...
    // --- [람다 4] 제거 실행 (멀티라인 대응) ---
    auto ProcessCommentRemoval = [&](Sci_Position start, Sci_Position end) {
        ...
        auto IsSpaceExtra = [](unsigned char c) { return c == ' ' || c == '\t' || c == 0xA0; };
    ...
    // --- [람다 5] 스타일 패밀리 판별 ---
    auto IsSameFamily = [](int s1, int s2) {
    ...
    // 블록 주석 범위 확장 보정
    int startStyle = (int)::SendMessage(hSci, SCI_GETSTYLEAT, startPos, 0);
    ...
    Sci_Line startLine = ::SendMessage(hSci, SCI_LINEFROMPOSITION, startPos, 0);
    Sci_Line endLine = ::SendMessage(hSci, SCI_LINEFROMPOSITION, endPos, 0);
    ...
    for (Sci_Position i = targetEnd - 1; i >= targetStart; --i) {
        int style = (int)::SendMessage(hSci, SCI_GETSTYLEAT, i, 0);
        if (IsCommentStyle(hSci, i, style)) {
            ...
            bool isBlock = IsBlockCommentStyle(style);
            Sci_Position limit = isBlock ? targetStart : curLineStart;
